...
\section*{Source Material and Section Payloads}
...
Section agents therefore write only to section-specific TeX files such as \texttt{tex/section\_payloads/ch07\_sec01\_section\_payloads.tex}. That constraint keeps each agent's responsibility local: one agent owns one section payload, and no agent silently mutates the whole document. The result is easier review, clearer attribution, and fewer accidental cross-section edits. It also supports incremental compilation, because a changed section can be regenerated, diffed, and repaired without forcing unrelated sections to be rewritten.
...
\begin{figure}[htbp]
\centering
\input{media/diagrams/ch07_sec01_section_payloads__media_req_e978c32217e8.tikz}
