...
The role map can be summarized as a chain of responsibility. Hypervision coordinates the run, outlining establishes the scaffold, section drafting turns scaffold into prose, gardening smooths the prose, diagram and design agents support presentation, references preserve source integrity, and assembly turns the accumulated work into a release-ready artifact. Each step narrows the gap between intent and publication while preserving a clear handoff to the next step.

That separation is especially important because the agents do not all operate at the same level of abstraction. Some roles are structural, some are editorial, and some are operational. The hypervisor and assembly agent sit at the edges of the workflow, one managing entry and progress, the other producing the final artifact. The outlining and section drafting agents do the core content work. The gardener, diagram, design, and references agents provide targeted support where the draft needs repair, illustration, presentation control, or source validation. By keeping those concerns distinct, the system avoids conflating authorship, maintenance, and production.

The result is a workflow that is easier to reason about and easier to revise. If a section is weak, the section agent revises it. If the structure is wrong, the outlining agent adjusts it. If the prose is coherent but awkwardly connected, the gardener improves it. If a figure is needed, the diagram agent supplies it. If a citation is missing, the references agent registers it. If the manuscript is ready to ship, the assembly agent compiles it. The machine therefore behaves less like a single author and more like a disciplined editorial organization.

Figure~\ref{fig:ch04_sec01_agent_roles} summarizes the role map and shows how the major agents relate to one another in the workflow.
...
\label{fig:ch04_sec01_agent_roles}
\end{figure}
